...
# '$4 !=""' -> Column "name" must not be empty, requierement in task 1.
...
BEGIN {FS=OFS="\t"; print "CREATE DATABASE IF NOT EXISTS shop_finder_db; USE shop_finder_db; DROP TABLE IF EXISTS shops; CREATE TABLE shops (id BIGINT NOT NULL, longitude double NOT NULL, latitude double NOT NULL, name varchar(255) NOT NULL, category varchar(255), homepage varchar(255), PRIMARY KEY (id)); DROP TABLE IF EXISTS points_of_interest; CREATE TABLE points_of_interest (id varchar(11) NOT NULL, longitude double NOT NULL, latitude double NOT NULL, name varchar(255) NOT NULL, PRIMARY KEY (id));"}$4 !=""{for (i=5; i<=NF; i++){gsub(/^yes$/,"", $i)}; print "INSERT INTO shops VALUES("$1","$2","$3",\""$4"\",\""$5"\",\""$6"\");"}END {print "LOAD DATA LOCAL INFILE 'graz_poi_complete.csv' INTO TABLE points_of_interest FIELDS TERMINATED BY '|' ENCLOSED BY '' LINES TERMINATED BY '\\r\\n' IGNORE 1 ROWS;"}